...
if (-not $env:RUNSIGIL_GATEWAY_URL) {
    $env:RUNSIGIL_GATEWAY_URL = "http://localhost:8080"
}
...
try {
    & $python "examples/protocol-gateway/live.py"
    if ($LASTEXITCODE -ne 0) { throw "The RunSigil protocol live proof failed." }
} finally {
    Pop-Location
}
